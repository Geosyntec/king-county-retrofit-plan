...
kc_reduced <- read_csv("Data/SubbasinAttributes_corplot1.csv", 
    col_types = cols(CrashDensity = col_double(), 
        MedianPavementCondition = col_double(), 
        SidewalkDensity = col_double()))
kc_reduced <- kc_reduced %>%  
  select(-c("ShellfishCount", "MedianSpawnerMortality"))
```
