...
abcUI <- function(id) {
  ns <- NS(id)
  tagList(
    leafletOutput(ns("map")), column(width = 12, checkboxInput('table_sel',label = "all",value = FALSE),
                                 DTOutput(ns("table")),
...
observe({
  ...
  leafletProxy('map') %>%
    ...
    addPolygons(data = idx, color = "yellow",layerId = 'highlighted',
...
#get map clicks
      ...
      output$map <- renderLeaflet({ leaflet(rv$selected) %>% addTiles()%>%
          addPolygons(layerId = ~SWSID,group = 'selected') %>%
          addPolygons(data = base.sf,color = 'grey',group = "base")
      })

      output$table <- renderDT({
        datatable(df, extensions="Scroller", style="bootstrap", selection = "single", class="compact", width="100%",
