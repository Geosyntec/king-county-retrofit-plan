...
dataset <- subbasin_data  %>% select_if(is.numeric) %>% na.omit() %>%  head(100)
...
df <- PF$UnicriterionNetFlows %>% `colnames<-`(criteria) %>% `rownames<-`(basins) %>% as.data.frame() %>% rownames_to_column('SWSID') %>%
  sample_n(10) %>% mutate(SWSID = SWSID %>% as.character())


df.long <- df %>% pivot_longer(-SWSID,names_to  = "Name") %>% left_join(metrics) %>% mutate_if()

apex(data = df.long, type = "column", mapping = aes(x = Name, y = value, fill = Goal)) %>% ax_facet_wrap(vars(SWSID), ncol = 1)  %>% ax_chart(stacked = TRUE)
