...
unet.long <- unet.df %>% pivot_longer(cols = -c(SWSID), names_to = "Name") %>% merge((metrics %>% select(Name,Goal,Subgoal)),by.y ="Name",by.x = "Name" )





rainbow.goals <- unet.df %>% group_by(Goal,Subgoal,SWSID,score) %>%
  dplyr::summarise(pos = sum(pos),neg=sum(neg),total = sum(value)) %>%
  select(c(Goal,Subgoal,SWSID,pos,neg)) %>%
  mutate(Goal = paste("Goal",Goal)) %>% pivot_longer(cols = where(is.numeric))

apex(data = unet.long %>% group_by(SWSID),
       type = "column",
     mapping = aes(x=Subgoal, fill = Name, y=value)) %>% ax_facet_wrap(vars(SWSID), ncol = 3) %>%   ax_chart(
      stacked = TRUE
     )
...
data_long <- PF$out_flows %>% dplyr::select(c(phi_minus, phi_plus, score)) %>%
