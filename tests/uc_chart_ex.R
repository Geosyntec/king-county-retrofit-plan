...
make_test_pf <- function(num=9) {
  dataset <-
    subbasin_data  %>% select_if(is.numeric) %>% na.omit() %>%  head(num)
...
out_tables <- function() {
  metrics %>% select(Metric_no, Name, Goal, Subgoal, Goal_Description,Subgoal_Description) %>%
...
df <- PF$UnicriterionNetFlows %>% `colnames<-`(c(criteria,'score_rank','score')) %>% `rownames<-`(basins) %>%
  as.data.frame() %>%
  rownames_to_column('SWSID') %>%
  mutate(SWSID = SWSID %>% as.factor())


df.long <- df %>% pivot_longer(-SWSID,names_to  = "Name") %>% left_join(metrics %>% select(
  Name, Goal, Subgoal
))
